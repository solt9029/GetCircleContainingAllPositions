    ...
    for ($first_index = 0; $first_index < count($positions); $first_index++) {
      for ($second_index = $first_index + 1; $second_index < count($positions); $second_index++) {
        for ($third_index = $second_index + 1; $third_index < count($positions); $third_index++) {
          $circle = getCircleContainingThreePositions([$positions[$first_index], $positions[$second_index], $positions[$third_index]]);
          $is_containing_all_positions = true;
          for ($index = 0; $index < count($positions); $index++) {
            if (!isWithinCircle($circle['center_position'], $circle['radius'], $positions[$index])) {
...
function isWithinCircle($center_position, $radius, $target_position) {
  if (dist([$target_position, $center_position]) <= $radius) {
    return true;
  }
  return false;
}
...
// 引数：$positionsは3点の座標
// 返値：3点を通る最小円の半径と中心点
function getCirclePassingThreePositions($positions) {
  $center_positions = [];
  $radius = 0;
  ...
  $radius = dist([$positions[0], $center_position]);
...
// 引数：$positionsは3点の座標
// 返値：3点を含む最小円の半径と中心点
function getCircleContainingThreePositions($positions) {
  $center_positions = [];
  $radius = 0;

  $lines = [dist([$positions[0], $positions[1]]), dist([$positions[1], $positions[2]]), dist([$positions[2], $positions[0]])];
  ...
  if (pow($lines[$max_line_index], 2) <= pow($lines[$other_line_indexes[0]], 2) + pow($lines[$other_line_indexes[1]], 2)) {
    $circle = getCirclePassingThreePositions($positions);
    $radius = $circle['radius'];
    $center_position = $circle['center_position'];
  } else { // 鈍角三角形の場合 
